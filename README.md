...
Convert text between 15 letter cases, turn any string into a clean URL slug, and generate numeronyms such as `i18n` — fast, free, and 100% client-side. Everything runs in your browser; your input is never sent over the network.
...
- **Case** — every conversion updates live as you type, each with its own copy button:
  `camelCase`, `PascalCase`, `snake_case`, `CONSTANT_CASE`, `kebab-case`, `Train-Case`, `dot.case`,
  `path/case`, `Title Case`, `Sentence case`, `no case`, `lower case`, `UPPER CASE`, `aLtErNaTiNg`
  and `iNVERSE cASE`.
- Smart word splitting on spaces, punctuation and case boundaries (`XMLHttpRequest` → `xml_http_request`),
  Unicode-aware (accented and Vietnamese letters are kept). Multi-line input is converted line by line.
- **Slugify** — separator (`-`, `_`, `.` or none), optional lowercasing, and optional diacritics removal
  (`é → e`, `ß → ss`, including Vietnamese `đ/Đ → d/D`). `&` becomes `and`.
- **Numeronym** — replaces every word longer than three characters with its numeronym
  (`internationalization` → `i18n`, `Kubernetes` → `K8s`).
- Sample text and one-click clear.
...
└── tool/                 # Case-converter–specific code
    ...
    ├── case.ts           # Word splitting and case conversions
    ├── slugify.ts        # Slug generation and diacritics removal
...
```bash
git clone https://github.com/techshield-tech/case-converter.git
cd case-converter
bun install
bun dev
```

Open the URL Vite prints — by default **http://localhost:5173/case-converter/**
(note the `/case-converter/` path, see [Base path](#base-path)).
...
| otherwise (default)     | `/case-converter/` | GitHub Pages                |

`BASE_PATH` should start and end with `/`, e.g. `/` or `/tools/case/`.
...
1. Go to [vercel.com/new](https://vercel.com/new) and import the
   `techshield-tech/case-converter` repository.
...
`example.com/tools/case/`), set the `BASE_PATH` environment variable in
**Settings → Environment Variables** to that path (e.g. `/tools/case/`) and redeploy.
...
`bun run build` with the default `/case-converter/` base, and publishes `dist/`.
...
<iframe
  id="case-converter"
  src="https://techshield-tech.github.io/case-converter/?embed=1&theme=dark"
  style="width: 100%; border: 0;"
  title="Case Converter & Slugify"
></iframe>

<script>
  const iframe = document.getElementById('case-converter');

  window.addEventListener('message', (event) => {
    const data = event.data;
    if (data?.slug !== 'case-converter') return;
...
| iframe → parent | `{ type: 'mmoall-tool:ready', slug: 'case-converter' }`          | Posted once on mount (embed mode only) |
| iframe → parent | `{ type: 'mmoall-tool:height', slug: 'case-converter', height }` | Posted whenever the document height changes (embed mode only) |
